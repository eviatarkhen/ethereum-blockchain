# Requirements Archive: v1.0 EduEthereum MVP
...
- [x] **STATE-04**: State transition function (apply transaction to world state)

### EVM

- [x] **EVM-01**: Stack machine with ~20 core opcodes (arithmetic, comparison, stack, memory, storage, flow control)
- [x] **EVM-02**: Gas metering (simplified — track and enforce gas limit)
- [x] **EVM-03**: CREATE opcode for contract deployment
- [x] **EVM-04**: CALL opcode for contract function calls

### Transactions

- [x] **TX-01**: Transaction structure (nonce, to, value, data, gas, signature)
- [x] **TX-02**: Transaction signing and sender recovery
- [x] **TX-03**: Transaction validation (nonce check, balance check, gas limit)
...
- [x] **LEARN-04**: `# SIMPLIFIED:` comment convention marking all deviations from real Ethereum
- [x] **LEARN-05**: Readable code with meaningful variable names for debugger inspection
...
| EVM-01 | Phase 3 | Complete |
| EVM-02 | Phase 3 | Complete |
| EVM-03 | Phase 3 | Complete |
| EVM-04 | Phase 3 | Complete |
| TX-03 | Phase 3 | Complete |
